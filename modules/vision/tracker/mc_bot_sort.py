...
class STrack(BaseTrack):
    ...
    _frame_dt = 1.0 / 30  # Updated by BoTSORT per frame
    ...
    def predict(self):
        ...
        self.mean, self.covariance = self.kalman_filter.predict(
            mean_state, self.covariance, dt=STrack._frame_dt)
    ...
    def multi_predict(stracks):
        if len(stracks) > 0:
            ...
            multi_mean, multi_covariance = STrack.shared_kalman.multi_predict(
                multi_mean, multi_covariance, dt=STrack._frame_dt)
    ...
    @property
    def tlbr(self):
        """Bounding box as (min x, min y, max x, max y)."""
        ret = self.tlwh.copy()
        ret[2:] += ret[:2]
        return ret
    ...
    @property
    def xywh(self):
        """Bounding box as (center x, center y, width, height)."""
        ret = self.tlwh.copy()
        ret[:2] += ret[2:] / 2.0
        return ret
...
class BoTSORT(object):
    ...
    Changes from original:
      - Re-ID removed (FastReIDInterface eliminated)
      - Frame dt computed from real timestamps for FPS-aware Kalman predictions
      - proximity_thresh and appearance_thresh used only for IoU gating
    """
    ...
    def __init__(self, args, frame_rate=30):
        ...
        # IoU gating thresholds (Re-ID removed)
        self.proximity_thresh = args.proximity_thresh

        self.gmc = GMC(method=args.cmc_method, verbose=[args.name, args.ablation])

        # Frame timing for dynamic Kalman dt
        self._last_frame_time = None
        self._nominal_dt = 1.0 / max(frame_rate, 1)

    def update(self, output_results, img):
        self.frame_id += 1

        # Compute real frame-to-frame dt
        now = time.monotonic()
        if self._last_frame_time is not None:
            dt = now - self._last_frame_time
            dt = max(0.001, min(dt, 1.0))  # Clamp: 1ms..1s
        else:
            dt = self._nominal_dt
        ...
        # IoU-only association (Re-ID removed)
